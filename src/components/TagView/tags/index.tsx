...
interface IProps {
  ...
  handleCloseAll: () => void;
  handleCloseOther: (tag: TagsItemType) => void;
}

const Tags: React.FC<IProps> = ({ tagList, handleCloseTag, handleCloseAll, handleCloseOther }) => {
  ...
  const handleCloseOtherTags = () => {
    setMenuVisible(false);
    currentTag && handleCloseOther && handleCloseOther(currentTag);
  };
    ...
    <div className={styles.tags_wrapper} ref={tagListRef}>
      <Scrollbars autoHide autoHideTimeout={1000} autoHideDuration={200}>
        {tagList.map((item, i) => (
          <div
            key={item.path}
            className={item.actived ? `${styles.item} ${styles.active}` : styles.item}
          ...
          >
            ...
            {i !== 0 && (
              <CloseOutlined
                ...
                onClick={() => handleCloseTag && handleCloseTag(item)}
              />
            )}
          </div>
        ))}
      ...
      {menuVisible ? (
        <ul
          ...
          ref={contextMenuRef}
        >
          {/* <li onClick={handleRefreshTag}>刷新</li> */}
